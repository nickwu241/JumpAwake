    ...
    if True:
        ...
        if len(faceFrames)<4:
           ...
           # cv2.rectangle(frame, (startX, startY), (endX, endY), (0, 0, 255), 2)
            cv2.rectangle(frame, (x0, y0), (x0+width, y0+height), (0, 0, 255), 2)
...
while True:
    ...
    if round(time.time(),2)%.5 == 0:    
        fps = frame_count / (time.time() - sample_time)
        print(str(fps))
        frame_count = 0
        sample_time = time.time()
    else:
    ...
    faceFrames = detectFaces2(frame)
    for faces in faceFrames:
    ...
    #     (0, 0, 255), 2)
    cv2.putText(frame, str(fps), (0, 10), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 0, 255), 2)
